    ...
    <section id="custody" aria-labelledby="custody-heading" className="border-b-[3px] border-ink">
      <div className="mx-auto max-w-[1440px] px-4 md:px-8 py-14 md:py-28 grid grid-cols-12 gap-6">
        ...
        <ol className="col-span-12 grid grid-cols-12 gap-4 md:gap-6">
          {CUSTODY.map((p) => (
            <li key={p.id} className={`col-span-12 sm:col-span-6 border-2 border-ink p-4 flex flex-col gap-3 bg-paper text-ink ${p.n === '07' ? 'lg:col-span-8' : 'lg:col-span-4'}`}>
              <p className="label flex justify-between">
                <span className="mono-wide">Pair {p.n}</span>
                <span>In rotation</span>
              </p>
              <div className="border-2 border-ink bg-paper aspect-[5/4] lg:aspect-auto lg:h-[300px] p-3 flex items-center justify-center overflow-hidden">
