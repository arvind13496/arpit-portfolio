...
export default function LotDialog({ lots, active, onRequestClose, onNavigate }) {
  ...
  return (
    <dialog
      ...
      className="m-0 md:m-auto w-full max-w-none md:max-w-[1100px] h-full md:h-auto md:max-h-[92vh] bg-paper text-ink border-0 md:border-[3px] border-ink p-0 overflow-auto"
    >
      <div className="sticky top-0 bg-paper border-b-[3px] border-ink flex items-center justify-between gap-4 px-4 md:px-6 h-12">
        <p className="label flex gap-4">
          <span className="mono-wide">Lot {active.lot}</span>
          <span>{active.format}</span>
          <span>{active.client}</span>
        </p>
        <button type="button" data-close onClick={onClose} className="label font-bold h-full px-3 border-l-[3px] border-ink -mr-4 md:-mr-6">
          <Q>CLOSE</Q>
      ...
      <div className="grid grid-cols-12 gap-6 p-4 md:p-6">
        <div className="col-span-12 lg:col-span-7 flex flex-col gap-6">
          ...
          <aside className="border-[3px] border-dashed border-ink p-4" aria-label="Self-critique">
        ...
        <figure className="col-span-12 lg:col-span-5">
          <div className="border-[3px] border-ink shadow-hard-8">
      ...
      <div className="border-t-[3px] border-ink grid grid-cols-2">
        <button type="button" onClick={() => onNavigate(prev.slug)} className="label font-bold p-4 text-left border-r-[3px] border-ink hover:bg-ink hover:text-lime focus-visible:bg-ink focus-visible:text-lime">
