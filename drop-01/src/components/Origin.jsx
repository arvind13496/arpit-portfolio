    ...
    <section id="kyc" aria-labelledby="kyc-heading" className="border-b-4 border-ink">
      <div className="mx-auto max-w-[1440px] px-4 md:px-8 py-14 md:py-28 grid grid-cols-12 gap-6">
        <div className="col-span-12">
          <p><span className="runhead">Lot 03 / KYC</span></p>
        ...
        <figure className="col-span-12 sm:col-span-6 md:col-span-3">
          <div className="border-4 border-ink shadow-hard-8 bg-paper">
        ...
        <div className="col-span-12 sm:col-span-6 md:col-span-4 border-[3px] border-ink bg-paper self-start">
          <p className="label bg-ink text-paper px-4 py-2">Know your customer</p>
          <dl className="p-4 text-sm">
            {ORIGIN.kyc.map(([k, v]) => (
              <div key={k} className="grid grid-cols-[minmax(0,12ch)_1fr] gap-2 py-1.5 border-b border-dotted border-ink">
                <dt className="label mono-cond">{k}</dt>
                <dd className="uppercase">{v}</dd>
              </div>
            ))}
          </dl>
          <p className="label px-4 py-2 border-t-[3px] border-ink flex justify-between">
            <span>Status</span>
